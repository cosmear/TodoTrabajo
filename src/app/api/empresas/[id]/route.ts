...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!companyId) return NextResponse.json({ success: false, error: 'ID inválido' }, { status: 400 });

    const connection = await pool.getConnection();

    try {
      // Get Company basic public info
      ...
      if (!company.is_active) {
         return NextResponse.json({ success: false, error: 'El perfil de esta empresa no está disponible.' }, { status: 404 });
      }
